...
<script>
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put(this.$store.state.globalhost+'product-service/seller/product/update',_this.ruleForm).then(function(resp){
                            if(resp.data.code == 0){
        ...
        created() {
            ...
            axios.get(this.$store.state.globalhost+'product-service/seller/product/findById/'+this.$route.query.id).then(function(resp){
